...
def construct(*, filters, blocks, components, attn_heads, use_nin, use_ln, extra_dims, x_dims=3):
    ...
    class FlowPosterior(Flow):
        def __init__(self, q_flow):
            ...
            def deep_processor(x, *, init, ema, dropout_p):
                ...
                processed_context = conv2d(tf.concat([this, that], 3), name='q_proj', num_units=32, init=init, ema=ema)
                for i in range(5):
                    processed_context = gated_resnet(
                        processed_context, name='q_c{}'.format(i),
                        a=None, dropout_p=dropout_p, ema=ema, init=init,
                        use_nin=False
                    )
                    processed_context = norm(processed_context, name='q_dqln{}'.format(i), ema=ema)
    ...
    class Generative(Flow):
        ...
        def sample(self, num_samples, flow_kwargs):
            y = tf.random_normal([num_samples, 64, 64, x_dims + extra_dims])
    ...
    dequant_flow = Dequantizer(Compose([
        CheckerboardSplit(),
        Norm(), MixLogisticCoupling(**dequant_coupling_kwargs), TupleFlip(),
        Norm(), MixLogisticCoupling(**dequant_coupling_kwargs), TupleFlip(),
        Norm(), MixLogisticCoupling(**dequant_coupling_kwargs), TupleFlip(),
        Norm(), MixLogisticCoupling(**dequant_coupling_kwargs), TupleFlip(),
...
def main():
    ...
    max_lr = 4e-4
    warmup_steps = 10000
    bs = 72 # set this to a smaller value if it can't fit on your GPU.
    ...
    def lr_schedule(step, *, decay=0.99995):
        ...
        if step < warmup_steps:
            ...
            return (1e-6)* (step - 280000) / warmup_steps
        else:
            return 1e-6
    ...
    if args.eval_checkpoint:
        ...
        ), seed=0, restore_checkpoint=args.eval_checkpoint, iw_samples=1024, total_bs=72)
        return
    ...
    train(
        ...
        logdir=f'~/logs/vflow_imagenet64_mix{components}_b{blocks}_f{filters}_h{attn_heads}_ln{int(use_ln)}_lr{max_lr}_bs{bs}_drop{dropout_p}_{floatx_str}',
